...
#' Check for string XMAS in word puzzle
#'
...
#' @param direction Integer between 1 and 9, indicating the index for the direction search
#'
...
check_xmas <- function(row, col, mc, mr, curr_letter, word_puzzle, direction = NULL) {
  ...
  apply(new_positions, \(x) { check_xmas(x[[1L]], x[[2L]], mc, mr, next_letter, word_puzzle, x[[3L]])}, MARGIN=1, simplify = FALSE)
}
...
check_x_mas <- function(row, col, mc, mr, word_puzzle) {
  ...
  if (nrow(matches) != 4L)
    return(FALSE)
  # Check if the indices upper left - lower right and upper right - lower left are S and M
