...
fn main() {
    ...
    let mut output_part1: u32 = 0;
    let mut output_part2: u32 = 0;
    ...
    for (game_index, game) in games.iter().enumerate() {
        ...
        if possible_game {
           output_part1 += game_index as u32 + 1;
           num_games += 1;
        }
        // PART 2:
        let minimum_draw: Draw = game.superset_of_all_draws().unwrap();
        let game_power = minimum_draw.get_power();
        // println!("The minimum required set is {:?}", minimum_draw);
        // println!("The power of this set is {}", game_power);
        output_part2 += game_power;
    }
    println!("PART 1: The number of possible games is {} with a score of {}", num_games, output_part1);
...
impl Game {
    ...
    // PART 2: For the given Vec<Draw>, find the minimum required cubes
    fn superset_of_all_draws(&self) -> Option<Draw> {
        // Make sure there is at least one draw in the game
...
impl Draw {
    ...
    // PART 2: Calculate the power of the draw
    fn get_power(&self) -> u32 {
        self.red * self.blue * self.green
    }

    // PART 2: Calculate the minimum required set, which is the maximum of each pair values
